package com.example.jhonsalya.evist.Sidebar;
...
public class SidebarAdapter extends BaseExpandableListAdapter{
    ...
    public int getChildrenCount(int groupPosition) {
        ...
        if (groupPosition == Constant.S_POS_PURCHASE) {
            // if group have child, return size of child
            childCount = this.listChild.get(this.listParent.get(groupPosition))
                    .size();
        }
        if (groupPosition == Constant.S_POS_SALES) {
            // if group have child, return size of child
            childCount = this.listChild.get(this.listParent.get(groupPosition))
                    .size();
        }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (groupPosition == 3){ //Cart
            holder.parentIcon.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_shopping_cart_black_24dp));
        }
        if (groupPosition == 4){ //Purchase
            holder.parentIcon.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_attach_money_black_24dp));
        }
        if (groupPosition == 5){ //Sales
            holder.parentIcon.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_attach_money_black_24dp));
        }
        if (groupPosition == 6){ //Logout
